...
inline void CopyToNewPageStd(Node *nptr, int low, int high, char *newbase, uint16_t cutoff, uint16_t &top) {//cutoff is potential head_comp ignored bytes
    for (int i = low; i < high; i++) {
        ...
        int key_len = oldhead->key_len;
            ...
            // delete[] presuf;
            int old_i = cutoff;
            int old_real_i = PV_SIZE - 1 - (cutoff % PV_SIZE);
            int new_i = 0;
            int new_real_i = 3;
            ...
            if (cutoff < PV_SIZE) {
                while (old_i < PV_SIZE && old_i < key_len) {
                    newhead->key_prefix[new_real_i] = oldhead->key_prefix[old_real_i];
                    old_i++; new_i++; new_real_i--; old_real_i--;
                }
                if (old_i == key_len) {
            ...
            if (new_i < PV_SIZE) {
                while (new_i < PV_SIZE && new_i < new_len) {
                    newhead->key_prefix[new_real_i] = suffix[suffix_i];
                    new_i++; suffix_i++; new_real_i--;
                }
            }
